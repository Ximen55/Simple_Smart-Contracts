...
var ui_label_Balance = new PIXI.Text("");
app.stage.addChild(ui_label_Balance);
ui_label_Balance.y=60;
...
function UpdateUI(){
	...
	ui_label_AccountInfo.text = "玩家ID: " + defaultAccount;
	if(!defaultAccount) {
    ...
    if (!error) {
        Balance = web3.fromWei(result,'ether').toFixed(4);
		ui_label_Balance.text = "玩家餘額: " + Balance + " ETH";
    } else {
        console.error(error);
    }
	});
...
var ui_bt_buytickets = new PIXI.Sprite.from("/assets/buy.png");
ui_bt_buytickets.x = 0;
ui_bt_buytickets.y = 160;
ui_bt_vote_dog.anchor.set(.5);
ui_bt_buytickets.interactive=true;
ui_bt_buytickets.buttonMode=true;
ui_bt_buytickets.sound = sound;
...
scene_Vote.addChild(ui_bt_buytickets)
...
function InitButtonEvent(){
    ...
    ui_bt_buytickets.on('click',function(){
        ui_bt_buytickets.sound.play();
        CallVote_2(1);
    });
                    ...
                    if(Balance >= num){
                        this.web3.eth.sendTransaction({
                            ...
                            if (error){
                              ...
                              console.log("賣方地址:",owner)
                              console.log("買方地址:",defaultAccount)
                            }
                          });
                        voteContractAtLocal.Buytickets(num,function(err){
                        });
                    }else{
                        alert("餘額不足");
                    }
...
function InitEventWatching(){
    ...
    var eventNewtickets = voteContractAtLocal.eventNewtickets();
    eventNewtickets.watch(function(err,res){
        if(!err){
            console.log("完成");
            UpdateUI();
        }
